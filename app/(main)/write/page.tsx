    ...
    <main className="px-5 pt-12 pb-6 max-w-sm mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-[#2B3A4A]">Writing</h1>
          <p className="text-xs text-[#2B3A4A]/40 font-medium mt-0.5">Poetry, stories, thoughts — just write</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-3">
          ...
          <p className="text-sm font-bold text-[#2B3A4A]/40">Nothing written yet</p>
          ...
          {writings.map(w => {
            ...
            return (
              <button
                ...
                className="w-full text-left bg-white rounded-3xl p-5 shadow-sm border border-[#EDE8F5] active:scale-[0.98] transition-transform"
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-extrabold text-white ${isMe ? 'bg-[#C4B5E0]' : 'bg-[#9B88C8]'}`}>
                      {(authorName || '?')[0].toUpperCase()}
                    </div>
                    <span className="text-xs font-bold text-[#2B3A4A]/60">{authorName || (isMe ? 'You' : 'Partner')}</span>
                  </div>
                  <span className="text-[10px] text-[#2B3A4A]/30 font-medium">{fmtRelative(w.created_at)}</span>
                </div>
                {w.title && (
                  <p className="font-extrabold text-[#2B3A4A] text-base mb-1">{w.title}</p>
                )}
                <p className="text-sm text-[#2B3A4A]/70 font-medium leading-relaxed line-clamp-3">{w.body}</p>
              </button>
            )
          })}
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-[#FBF8F4] flex flex-col">
          <div className="flex items-center justify-between px-5 pt-12 pb-4 border-b border-[#EDE8F5]">
            <button
              ...
              className="text-[#9B88C8] font-bold text-sm"
            >
              Cancel
            </button>
            <h2 className="font-extrabold text-[#2B3A4A]">New writing</h2>
            <button
              onClick={handleSave}
              disabled={saving || !body.trim()}
              className="text-[#9B88C8] font-bold text-sm disabled:opacity-40"
            >
              {saving ? 'Posting…' : 'Post'}
            </button>
          </div>
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            <input
              ...
              className="w-full text-xl font-extrabold text-[#2B3A4A] bg-transparent outline-none placeholder:text-[#2B3A4A]/20"
            />
            <textarea
              value={body}
      ...
      {expandedWriting && (
        <div className="fixed inset-0 z-50 bg-[#FBF8F4] flex flex-col">
          <div className="flex items-center justify-between px-5 pt-12 pb-4 border-b border-[#EDE8F5]">
            <button onClick={() => setExpanded(null)} className="text-[#9B88C8] font-bold text-sm">
              ← Back
            </button>
            <div />
            {expandedWriting.slot === slot && (
              <button
          ...
          <div className="flex-1 overflow-y-auto px-5 py-6 space-y-4">
            <div className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-extrabold text-white ${expandedWriting.slot === slot ? 'bg-[#C4B5E0]' : 'bg-[#9B88C8]'}`}>
                {((expandedWriting.slot === slot ? myName : partnerName) || '?')[0].toUpperCase()}
              </div>
              <span className="text-sm font-bold text-[#2B3A4A]/60">
